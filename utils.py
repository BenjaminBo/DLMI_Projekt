...
class DataPrepUtils():
    ...
    def split_class(data_list:List, training_to_test_ratio:float=0.9, training_to_val_ratio:float=0.8) -> Tuple[List, List, List]:
        ...
        test = data_list[int(training_to_test_ratio*(len(data_list)-1)):] #test

        random.shuffle(train_val)
        random.shuffle(test)

        val = train_val[int(training_to_val_ratio*(len(train_val)-1)):] #train
        training = train_val[:int(training_to_val_ratio*(len(train_val)-1))] #val
